...
test('outer image combines Node with the nested Podman controller', () => {
    ...
    for (const executable of ['/usr/bin/podman', '/usr/bin/fuse-overlayfs', '/usr/bin/pasta', '/usr/local/bin/node']) {
...
test('runtime contract and smoke describe nested Podman v2', () => {
    assert.equal(sources.runtimeContract.path, '/opt/roboteam-runtime/contract-v2');
    assert.equal(sources.runtimeContract.content, 'roboteam-runtime-v2\n');
    for (const source of [dockerfile, smoke]) {
        assert.match(source, /roboteam-runtime-v2/);
...
test('publication remains native, candidate-first, and digest assembled', () => {
    ...
    assert.match(workflow, /--privileged/);
